...
import PhoneMockup, { BoxerTrainingScreen, BoxerRecoveryScreen, BoxerProgressScreen, CoachRosterScreen, CoachAnalyticsScreen, CoachInsightsScreen, VideoScreenProps } from "./PhoneMockup";
...
const ParallaxPhone = ({
  ...
  return <motion.div initial={{
    ...
    duration: 0.6
  }} className="flex flex-col items-center" onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}>
      <motion.div style={{
      y: yTransform
    }} whileHover={{
      y: -10,
      ...
      damping: 20
        ...
        <PhoneMockup>
          {feature.hasVideo ? <ScreenComponent isPlaying={isPlaying} /> : <ScreenComponent />}
        </PhoneMockup>
      </motion.div>
...
const FeatureGrid = () => {
  ...
  return <section id="features" className="py-24 md:py-32 relative overflow-hidden">
      {/* Background accent */}
      ...
      <div className="container mx-auto px-6" ref={containerRef}>
        ...
        <motion.div key={`${customerType}-carousel`} initial={{
          opacity: 0,
          y: 20
        }} animate={{
          opacity: 1,
          ...
          duration: 0.4
            ...
            loop: true
                ...
                const ScreenComponent = feature.ScreenComponent;
                return <CarouselItem key={feature.title} className="flex justify-center">
                          <div className="flex flex-col items-center px-4">
                            <div className="mb-6">
                              <PhoneMockup>
                                {feature.hasVideo ? <ScreenComponent isPlaying={index === currentSlide} /> : <ScreenComponent />}
                              </PhoneMockup>
                            </div>
                            <div className="text-center max-w-xs">
                              <h3 className="text-xl font-bold mb-2 text-foreground">
                ...
                <div className="flex justify-center gap-3 mt-8">
                  {phoneFeatures.map((_, i) => <button key={i} className={`w-2.5 h-2.5 rounded-full transition-all duration-200 ${i === currentSlide ? 'bg-primary scale-110' : 'bg-white/20 hover:bg-white/40'}`} onClick={() => carouselApi?.scrollTo(i)} aria-label={`Go to slide ${i + 1}`} />)}
                </div>
              </motion.div> :
        // Desktop: Grid with Parallax
        <motion.div key={`${customerType}-grid`} initial={{
          opacity: 0,
          y: 20
        }} animate={{
          opacity: 1,
          ...
          duration: 0.4
                ...
                {phoneFeatures.map((feature, index) => <ParallaxPhone key={feature.title} feature={feature} yTransform={yTransforms[index]} isInView={isInView} isPlaying={index === activeVideoIndex} onMouseEnter={() => setHoveredIndex(index)} onMouseLeave={() => setHoveredIndex(null)} />)}
              </motion.div> :
        // Benefits List for Promoter and Fan
        <motion.div key={`${customerType}-benefits`} initial={{
          opacity: 0,
          y: 20
        }} animate={{
          opacity: 1,
